...
int between_time_in_s = 10 * 60; // 10 * 60
const unsigned char start_followline_once[] = {0x01, 0x06, 0x00, 0x01, 0x00, 0x00, 0xD8, 0x0A};
...
{  
  ...
  fd_car = UART0_Open(fd_car,argv[1]);                    //打开串口，返回文件描述符  
  do
  {  
  ...
  while(1)
  {
    printf("start_followline_once %d\r\n", ++send_num);
    UART0_Send(fd_car, (char*)start_followline_once, sizeof(start_followline_once));
    sleep(between_time_in_s);
  }
  UART0_Close(fd_car);
}
